...
# Define structured state for our flow
...
class GuidedGeneratorFlow(Flow[ResearchFlowState]):
    """
    Flow that orchestrates the complete guide generation process

    Flow steps:
    1. receive_user_input - Accept and validate user input
    ...
    def receive_user_input(self) -> str:
        ...
        if self.state.document_paths:
            sources_provided.append("Document")
        if self.state.webpage_links:
            sources_provided.append("Web Page")
    ...
    def run_research_crew(self, prev_output) -> str:
        ...
        Research crew uses a manager agent to coordinate 4 specialists
        who gathers information from different sources types. The manager then
